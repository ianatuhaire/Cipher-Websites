	...
	<figure class="woocommerce-product-gallery__wrapper">
		...
		if ( has_post_thumbnail() ) {
			$image = glb__get_image_resized( array(
				...
				'crop'     => glb__option( 'product__imageSizeCrop' ),
				'atts'     => $attributes
			) );

			$thumbnail       = glb__get_image_resized( array(
				'image_id' => $post_thumbnail_id,
				'size'     => glb__option( 'product__thumbnailSize' ),
				'crop'     => glb__option( 'product__thumbnailSizeCrop' )
			) );
			...
			$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'glb' ) );
			$html .= '</div>';
		}
