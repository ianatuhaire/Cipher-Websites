...
class Glb__Sample_Data_Worker
{
	...
	public function download() {
		...
		if ( isset($_POST['file']) && glb__initialize_filesystem_api() ) {
			$source           = sprintf( 'http://live.linethemes.com/data/%s/%s', GLB__ID, $_POST['file'] );
		...
		if ( isset( $_POST['file'] ) && glb__initialize_filesystem_api() ) {
			...
			if ( is_file( $file ) && unzip_file( $file, $this->path() ) ) {
				...
				if ( ! is_file( $data_file ) ) {
					throw new Exception( sprintf( esc_html__( 'Sample data file not found: %s', 'glb' ), $data_file ) );
				}
				...
				while ( ! feof( $file_handle ) ) {
					...
					if ( empty( $record ) || empty( $tables[ $record['table']] ) ) {
						continue;
					}
					...
					if ( $record['table'] == 'options' ) {
					    $wpdb->replace($tables[ $record['table'] ], $row);
                    } else {
                        if ( empty( $insert_queues[$record['table']] ) ) {
                            $insert_queues[$record['table']] = [
                                'values' => [],
                                'fields' => array_keys($row)
                            ];
                        }

                        $insert_queues[$record['table']]['values'][] = $row;
                    }
				}

				foreach ( $insert_queues as $table => $data ) {
                    $wpdb->query( "TRUNCATE TABLE {$tables[ $table ]}" );
                    $this->do_insert($tables[ $table ], $data['values'], $data['fields']);
                }
	...
	public function do_insert ($table, $rows, $fields) {
	    global $wpdb;

        $fields = join('`, `', $fields);
        $chunkedRows = array_chunk($rows, 50);

        foreach ($chunkedRows as $rows) {
            $params = array_reduce($rows, function ($data, $row) {
                $holders = array_map(function () {
                    return '%s';
                }, $row);

                $data['holders'][] = sprintf('(%s)', join(', ', $holders));
                $data['values'] = array_merge($data['values'], array_values($row));

                return $data;
            }, ['holders' => [], 'values' => []]);

            $holders = implode(', ', $params['holders']);
            $preparedQuery = $wpdb->prepare(
                "INSERT INTO {$table} (`{$fields}`) VALUES {$holders}", $params['values']
            );
	...
	public function thumbnails() {
		...
		if ( ! glb__initialize_filesystem_api() ) {
			$this->response( array(
				'code'    => 403,
				'message' => esc_html__( 'Invalid files & folders permissions', 'glb' )
			) );
		}
		...
		if ( ! is_dir( $this->path( 'uploads' ) ) ) {
			$this->response( array(
				'code'    => 500,
				'message' => esc_html__( 'Cannot extract the sample data media files', 'glb' ),
		...
		$this->response( array(
			'code'    => 200,
			'message' => esc_html__( 'Thumbnails generation successfully!', 'glb' )
		) );
	}
	...
	public function finish() {
		...
		if ( isset( $_POST['package'] ) && glb__initialize_filesystem_api() ) {
