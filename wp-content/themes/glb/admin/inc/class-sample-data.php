...
class Glb__Sample_Data
{
	...
	public function admin_menu() {
		$page_title = esc_html__( 'Sample Data Installation', 'glb' );
		$menu_title = esc_html__( 'Sample Data', 'glb' );
	...
	public function enqueue( $page ) {
		if ( $page == $this->menu ) {
			...
			wp_localize_script( 'sample-data', '_sampleDataI18n', array(
				'confirmInstallation' => esc_html__( 'Attention!!! Your existing data will be removed when install sample data. Are you sure you want to install sample data?', 'glb' )
			) );

			wp_localize_script( 'sample-data', '_sampleDataInfo', array(
				'nonce'    => wp_create_nonce( 'sample_data_installation' ),
				'packages' => apply_filters( 'glb/sample-datas', array() ),
