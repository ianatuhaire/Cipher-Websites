...
add_filter( 'acf/prepare_field/name=sidebar', function ( $field ) {
	...
	$field['choices'] = array();
	$field['choices']['default'] = esc_html__( 'Use Theme Default', 'glb' );
...
function glb__override_sidebar_position( $position ) {
	$object = get_post();
...
add_filter( 'glb__sidebar_position', 'glb__override_sidebar_position', 99 );


function glb__override_sidebar_id( $id ) {
...
add_filter( 'glb__sidebar_id', 'glb__override_sidebar_id', 99 );
