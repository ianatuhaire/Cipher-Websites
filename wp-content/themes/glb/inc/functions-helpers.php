 ...
 * @return  mixed
 */
function glb__option( $id ) {
	global $wp_filesystem;
	static $options, $options_default;

	if ( empty( $options ) && glb__initialize_filesystem_api() ) {
		$options_file = get_theme_file_path( 'admin/options.json' );
		$options = [];

		if ( $wp_filesystem->exists( $options_file ) ) {
			$options = json_decode( $wp_filesystem->get_contents( $options_file ), true );
		}
	}

	if ( empty( $options_default ) ) {
		$options_default = apply_filters( 'glb__customize_settings', array() );
		
		foreach ( $options_default as $key => $params ) {
			if ( ! isset( $options[ $key ] ) && isset( $params['default'] ) ) {
				$options[ $key ] = $params['default'];
			}
		}
	}

	return get_theme_mod( $id, isset( $options[ $id ] ) ? $options[ $id ] : null );
}
 ...
 * @return  strings
 */
function glb__current_post_type() {
 ...
 * @since   1.0.0
 */
function glb__logo( $prefix = '', $attrs = array() ) {
	$logo_key = 'logo';
	...
	$logo = (array) glb__option( $logo_key );
	$logo_retina = (array) glb__option( $logo_retina_key );
	...
	printf( '<img %s />', glb__attributes( $attributes ) );
}


function glb__social_icons( $args ) {
	...
	$positions = glb__option( 'global__social__positions' );

	if ( in_array( $args['location'], $positions ) ) {
		$icons = glb__option( 'global__social__icons' );
		$output = array();
 ...
 * @since   1.0.0
 */
function glb__attributes( $attrs ) {
	$attributes = array();
 ...
 * @since   1.0.0
 */
function glb__google_fonts() {
	// Try to retrive the font list from the cache
	$cached = get_transient( 'glb-fonts' );
	...
	if ( ! $cached ) {
		...
		if ( is_array( $remote_content ) && isset( $remote_content['body'] ) ) {
			...
			set_transient( 'glb-fonts', $cached, 86400 );
		}
	}
 ...
 * @since   1.0.0
 */
function glb__initialize_filesystem_api( $url = '', $action = -1, $name = '_wpnonce' ) {
 ...
 * @return  array
 */
function glb__get_image_resized( $params = array() ) {
	...
	if ( is_string( $thumb_size ) && ( ( ! empty( $_wp_additional_image_sizes[ $thumb_size ] ) && is_array( $_wp_additional_image_sizes[ $thumb_size ] ) ) || in_array( $thumb_size, array(
		...
		if ( is_array( $thumb_size ) ) {
			...
			$p_img = glb__image_resize( $attach_id, null, $thumb_size[0], $thumb_size[1], $params['crop'] );
			...
			if ( ! empty( $attachment ) ) {
				...
				if ( $p_img ) {
					...
					$thumbnail = '<img ' . glb__attributes_stringify( $attributes ) . ' />';
				}
			}
		}
	} else {
	...
	return apply_filters( 'glb__get_image_resized', array(
		'thumbnail' => $thumbnail,
		'thumbnail_raw' => $p_img,
		'large' => $p_img_large,
	), $attach_id, $params );
 ...
 * @return array
 */
function glb__image_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
 ...
 * @return string
 */
function glb__attributes_stringify( $attributes ) {
	$atts = array();
